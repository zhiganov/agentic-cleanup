...
- **Runtime selection:** `AGENTIC_CLEANUP_RUNTIME=claude|opencode|all` controls which command copies an installer publishes. The shared payload records the selected runtimes in `installed-runtimes`, and the command verifies exactly those copies.
